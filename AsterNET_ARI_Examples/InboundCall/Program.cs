...
namespace InboundCall
{
    ...
    {
        /// <summary>
        /// Application name used in the dialplan to route inbound calls to the application to.
        /// See the dialplan segment below.
        /// </summary>
        private const string applicationName = "cvegso_inbound_cc";
        ...
        /// 
        /// NOTE: this endpoint should be provisioned in the dialplan (extensions_custom.conf) as follows:
        /// 
        ///    [cvegso-cc]
        ///    exten => 6789,1,NoOp()
        ///    exten => 6789,n,Stasis(cvegso_inbound_cc)
        ///    exten => 6789,n,Hangup()
        ///    
        /// </summary>
        private const string dialInUri = "SIP/6789";
        ...
        private static void SetupAriClient()
        {
            ...
            ariClient.OnStasisEndEvent += AriClient_OnStasisEndEvent;
        }
        ...
        /// <summary>
        /// Event handler to process new incoming calls.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private static void AriClient_OnStasisStartEvent(IAriClient sender, StasisStartEvent e)
        {
            string logLabel = nameof(AriClient_OnStasisStartEvent);

            try
            {
                Console.WriteLine($"{logLabel} - Application: {e.Application}, Type: {e.Type}, ChannelId: {e.Channel.Id}, ChannelState: {e.Channel.State}");
                ...
                customerChannel = e.Channel;

                /*
                 * Let's start playing the welcome message.
                 */
            ...
            {
                Console.WriteLine($"{logLabel} - Failed to handle the customer call. Reason: {ex.Message}");
            }
        }

        /// <summary>
        /// Event handler to process incoming call ends.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private static void AriClient_OnStasisEndEvent(IAriClient sender, StasisEndEvent e)
        {
            string logLabel = nameof(AriClient_OnStasisEndEvent);

            try
            {
                Console.WriteLine($"{logLabel} - Application: {e.Application}, Type: {e.Type}, ChannelId: {e.Channel.Id}, ChannelState: {e.Channel.State}");

                if (customerChannel != null && e.Channel.Id == customerChannel.Id)
                {
                    Console.WriteLine($"{logLabel} - Customer call with ChannelId: {e.Channel.Id} left the application (e.g. terminated)");
            ...
            {
                Console.WriteLine($"{logLabel} - Failed to process the event. Reason: {ex.Message}");
            }
        }
        ...
        private static void DestroyBridge(Bridge bridge)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"{logLabel} - No need to destroy the bridge");
                }
            }
            catch (Exception ex)
            {
